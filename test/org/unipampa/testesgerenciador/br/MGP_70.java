...
public class MGP_70 {
    ...
    @Test
    public void notificaEmailGerente() throws Exception {
        try {
            ...
            if (!driver.findElement(By.className("hP")).getText().equals("Gerenciador Pampatec - Notificação")) {
                throw new Exception("Email de notificação do gerente não recebido!");
            }

            Connection.updateResults("Notificação email gerente", null, TestLinkAPIResults.TEST_PASSED, TESTLINK_KEY);
            ...
            TestingSupport.saveScreenshotError(driver, System.getProperty("user.dir") + System.getProperty("file.separator")
                    ...
                    + "evidenciaserro", "Notificação email gerente");

            Connection.updateResults("Notificação email gerente", e.getMessage(), TestLinkAPIResults.TEST_FAILED, TESTLINK_KEY);
